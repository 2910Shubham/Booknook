...
interface UseHighlighterProps {
    ...
    addAnnotation: (annotation: HighlightAnnotation) => void;
    redraw: () => void;
}
...
export function useHighlighter({
    ...
    useEffect(() => {
        ...
        const handleMove = (clientX: number, clientY: number) => {
            ...
            const height = Math.abs(startRef.current.y - point.y);
            drawHighlightPreview(ctx, color, x, y, width, height);
        };
        ...
        const handleEnd = (clientX: number, clientY: number) => {
            ...
            const ny = Math.min(startRef.current.ny, point.ny);
            const width = Math.abs(startRef.current.nx - point.nx);
            const height = Math.abs(startRef.current.ny - point.ny);
